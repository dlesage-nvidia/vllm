...
def worker(args: argparse.Namespace) -> None:
    ...
    if device_normalize and not torch.equal(raw_processor_pixels, production_pixels):
        raise RuntimeError("head production processor pixels differ from raw probe")
    ...
    if grid_values != [[16, 36, 64]]:
        raise RuntimeError(
            "processor did not realize the configured 1024x576-per-frame pixel "
            f"budget at the expected 32-frame grid: {grid_values}"
        )
    ...
    if (resized_width, resized_height) != PIXEL_BUDGET:
        raise RuntimeError(
            f"processor realized {(resized_width, resized_height)} rather than "
            f"the configured {PIXEL_BUDGET} per-frame pixel budget"
        )
    tensor_artifact = args.output.with_suffix(".tensors.pt")
    if args.output.exists() or tensor_artifact.exists():
        raise FileExistsError(
            f"refusing to overwrite worker evidence: {args.output}, "
            f"{tensor_artifact}"
        )
    torch.save(
        {
    ...
    result = {
        "schema": "pynv-endpoint-pixel-parity-worker-v2",
...
def orchestrator(args: argparse.Namespace) -> None:
    ...
    result: dict[str, Any] = {
        ...
        "video": {"path": str(args.video), "sha256": sha256_file(args.video)},
        "frames": FRAMES,
        "model": MODEL,
        "revision": REVISION,
    ...
    for variant, commit in COMMITS.items():
        ...
        if bytecode_paths:
            raise RuntimeError(
                f"source contains ignored bytecode/cache at {variant}: "
                f"{bytecode_paths[:20]}"
            )
        source_harness = (
        ...
        variant_result["worker_result_artifact"] = {
        ...
        variant_result["source"] = {
            "commit": commit,
            "tree": output(["git", "-C", str(args.root), "rev-parse", "HEAD^{tree}"]),
            "status": status,
            "experiment_harness_path": str(source_harness),
    ...
    parity_fields = {
        ...
        "processor_resolved_resolution_exact_all_variants": (
            len(
                {
                    (
                        variant_result["processor"]["processed_width"],
                        variant_result["processor"]["processed_height"],
                    )
                    for variant_result in variant_results.values()
                }
            )
        ...
        "processor_resolved_pixels_within_budget_all_variants": all(
            variant_result["processor"]["processed_pixels_per_frame"]
            <= MAX_PIXELS_PER_FRAME
            for variant_result in variant_results.values()
        ),
        "processor_realized_configured_1024x576_budget_all_variants": all(
