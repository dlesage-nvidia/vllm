...
def compress_result(path: Path, uncompressed_sha256: str) -> Path:
    ...
    if sha256_gzip_payload(temporary) != uncompressed_sha256:
        temporary.unlink()
        raise RuntimeError(f"compressed result verification failed: {path}")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="resume a validated partial matrix and gzip each completed result",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.resume:
        ...
        if manifest.get("schema") != "pynv-gpu-resize-pr1-matrix-v1":
            raise RuntimeError("resume manifest schema mismatch")
        if manifest.get("commits") != COMMITS or manifest.get("trees") != TREES:
            raise RuntimeError("resume manifest commit/tree mismatch")
        if manifest.get("gpu_identity") != gpu_identity:
            raise RuntimeError("resume GPU identity mismatch")
        prior_cells = list(manifest.get("cells", []))
        manifest.setdefault("resume_history", []).append(
            {
                "resumed_utc": utc_now(),
                "reason": "filesystem filled while writing r04-pr1; invalid partial result removed",
        ...
        manifest["cells"] = [
            cell for cell in prior_cells if cell.get("status") == "passed"
        ]
        manifest["status"] = "running"
        manifest["lock_status"] = lock_status
    else:
        manifest = {
    ...
    for cell in manifest["cells"]:
        ...
        original_path = Path(cell["result"])
        result_path = (
            original_path
            if original_path.exists()
            else original_path.with_suffix(original_path.suffix + ".gz")
        )
        if not result_path.is_file():
            raise FileNotFoundError(f"missing completed result during resume: {result_path}")
        expected_uncompressed = cell.get(
            "result_uncompressed_sha256", cell.get("result_sha256")
        )
        if result_path.suffix == ".gz":
            if sha256_gzip_payload(result_path) != expected_uncompressed:
                raise RuntimeError(f"resume result payload hash mismatch: {result_path}")
            cell["result_uncompressed_sha256"] = expected_uncompressed
            cell["result_sha256"] = sha256_file(result_path)
            cell["result_compression"] = "gzip"
            cell["result"] = str(result_path)
        elif sha256_file(result_path) != expected_uncompressed:
            raise RuntimeError(f"resume result hash mismatch: {result_path}")
        result = read_result(result_path)
        monitor_path = Path(cell["monitor"])
        server_log = Path(cell["server_log"])
        ...
        if metrics != cell["metrics"]:
            raise RuntimeError(f"resume metrics mismatch for r{rep:02d}-{variant}")
        results_by_pair[(rep, variant)] = result
        print(f"RESUME-VALIDATED r{rep:02d}-{variant}", flush=True)
    write_json(manifest_path, manifest)
    try:
        for rep, concurrencies, variants in SCHEDULE:
            for variant in variants:
                if (rep, variant) in results_by_pair:
                    print(f"SKIP r{rep:02d}-{variant} already validated", flush=True)
                    continue
                ...
                result_uncompressed_sha256 = sha256_file(result_path)
                compressed_result_path = compress_result(
                    result_path, result_uncompressed_sha256
                )
                cell.update(
                    {
                        "status": "passed",
                        "source_after": source_record(args.source_root, variant),
                        "result": str(compressed_result_path),
                        "result_compression": "gzip",
                        "result_uncompressed_sha256": result_uncompressed_sha256,
                        "result_sha256": sha256_file(compressed_result_path),
