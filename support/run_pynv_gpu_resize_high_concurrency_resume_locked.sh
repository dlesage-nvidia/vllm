...
runner=/home/ubuntu/work/run_pynv_gpu_resize_high_concurrency_matrix_resume.py
...
initial_runner_log=$artifact_root/.$run_name-runner.log
runner_log=$artifact_root/.$run_name-resume-runner.log
...
runner_sha256=3b953778ed47fef5aff8e10f7ddf4e4f4bbe9f1a2d9e659fac7d58d083761835
...
setsid timeout --signal=INT --kill-after=180s 18000s \
    ...
    --gpu-label "$gpu_label" \
    --port 18700 \
    --resume >"$runner_log" 2>&1 &
workload_pid=$!
wait "$workload_pid"
workload_pid=

mv "$test_log" "$results/gpu-tests.log"
mv "$initial_runner_log" "$results/runner-initial.log"
mv "$runner_log" "$results/runner-resume.log"
sha256sum "$results/gpu-tests.log" "$results/runner-initial.log" \
    "$results/runner-resume.log" \
