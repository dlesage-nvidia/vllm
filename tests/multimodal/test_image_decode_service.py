...
def test_ready_queue_is_partitioned_across_idle_decoder_workers():
    calls: list[list[bytes]] = []
    lock = threading.Lock()
    ...
    service = get_nvimagecodec_decode_service(image_io)
    encoded = [b"\xff\xd8" + bytes([index]) for index in range(20)]
    with service._cond:
        futures = [service.submit(image_io, [item]) for item in encoded]
    results = [future.result(timeout=2)[0] for future in futures]

    assert calls == [encoded[:10], encoded[10:]]
    assert service.snapshot_stats().batch_widths == {10: 2}
    for result in results:
        result.media.close()


def test_ready_queue_uses_full_claim_when_only_one_decoder_is_idle():
    direct_started = threading.Event()
    coalesced_started = threading.Event()
    release_direct = threading.Event()
    coalesced_widths: list[int] = []
    ...
    def load(items: list[bytes]):
        if items == [b"direct"]:
            ...
            coalesced_widths.append(len(items))
            coalesced_started.set()
        return [_result(item) for item in items]
    ...
    service = get_nvimagecodec_decode_service(image_io)
    direct = service.submit(image_io, [b"direct"])
    assert direct_started.wait(timeout=2)

    encoded = [b"\xff\xd8" + bytes([index]) for index in range(20)]
    with service._cond:
        futures = [service.submit(image_io, [item]) for item in encoded]
    assert coalesced_started.wait(timeout=2)
    release_direct.set()
    direct_result = direct.result(timeout=2)
    results = [future.result(timeout=2)[0] for future in futures]

    assert coalesced_widths == [20]
    assert service.snapshot_stats().batch_widths == {20: 1}
    direct_result[0].media.close()
    for result in results:
        result.media.close()
