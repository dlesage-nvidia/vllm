...
"""Benchmark Qwen2.5-VL video captioning through a fresh ``vllm serve``.
...
SERVED_MODEL_NAME = "qwen2.5-vl-video-throughput"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=positive_int,
        default=32,
        help="Exact Qwen2.5-VL sampling target (default: 32)",
    )
    parser.add_argument(
        "--video-pixel-budget",
        type=dimensions,
        ...
        help=(
            "Per-sampled-frame Qwen2.5-VL max_pixels budget "
            "(default: 1024x576)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=9216,
        help=(
            "Scheduler and encoder-cache token budget (default: 9216)"
        ),
    )
    parser.add_argument(
        "--max-num-seqs",
...
def expected_video_work(
    ...
    if video_kwargs is None:
        ...
        reason = "Qwen2.5-VL equal min/max frame clamp"
    elif not isinstance(original_fps, (int, float)) or original_fps <= 0:
        reason = "source fps unavailable for Qwen2.5-VL sampling calculation"
        ...
        reason = "Qwen2.5-VL fps sampling calculation"
...
def video_mm_processor_kwargs(args: argparse.Namespace) -> dict[str, int]:
    ...
    # Qwen2.5-VL applies max_pixels to each frame. Qwen3-VL instead expects a
    # total-video budget, which is why the original harness multiplied by the
    # frame count.
    return {"max_pixels": width * height}
        ...
        try:
            self.connection.request(
                ...
                headers={
                    ...
                    "User-Agent": "vllm-qwen25-vl-e2e-persistent-benchmark/1",
                },
            )
...
def batch_aggregate(
    ...
    return {
        ...
        "video_megapixel_estimate_method": (
            "sum(estimated Qwen2.5-VL sampled frames * externally probed encoded "
...
def run(args: argparse.Namespace) -> int:
    ...
    limit_mm_per_prompt = video_limit_mm_per_prompt(args)
    result: dict[str, Any] = {
        "schema": "vllm-qwen25-vl-video-e2e-throughput-v1-persistent-http",
        "status": "running",
        "started_at": started_at,
        "configuration": {
            ...
            "video_pixel_budget": {
                ...
                "server_max_pixels_per_frame": mm_processor_kwargs["max_pixels"],
                "reference_max_pixels_total": (
                    pixel_budget_width * pixel_budget_height * args.frames
                ),
                "resize_policy": (
                    "Qwen2.5-VL preserves source aspect ratio and rounds each "
                    "frame to its spatial factor within the per-frame ceiling"
                ),
            },
