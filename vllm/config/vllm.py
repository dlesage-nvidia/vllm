...
class VllmConfig:
    ...
    def _validate_nvimagecodec_frontend(self) -> None:
        ...
        if (
            mm_config.get_limit_per_prompt("image") != 1
            or mm_config.get_limit_per_prompt("video") != 0
        ):
            # The pinned-output budget must never make one request wait for a
            # later image while that request retains an earlier output.
            raise ValueError(
                "The nvimagecodec image backend requires "
                '--limit-mm-per-prompt \'{"image": 1, "video": 0}\'.'
            )
        mm_config._validate_nvimagecodec_processor_kwargs(
            mm_config.mm_processor_kwargs or {}
        )
        if (
